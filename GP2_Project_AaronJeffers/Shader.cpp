...
void Shader::initialise(const std::string& filename)
{
	...
	shaders[0] = CreateTextureShader(LoadTextureShader("..\\res\\shader.vert"), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateTextureShader(LoadTextureShader("..\\res\\shader.frag"), GL_FRAGMENT_SHADER); // create fragment shader
	...
	CheckError(program, GL_LINK_STATUS, true, "Error: Shader program linking failed"); // cheack for error

	glValidateProgram(program); //check the entire program is valid
	CheckError(program, GL_VALIDATE_STATUS, true, "Error: Shader program not valid");

	uniforms[TRANSFORM_UNIFORMS] = glGetUniformLocation(program, "transform"); // associate with the location of uniform variable within a program
}
...
void Shader::Update(const Tran& transform, const GameCam& camera)
{
	glm::mat4 mvp = camera.GetViewProjection() * transform.GetModel();
	glUniformMatrix4fv(uniforms[TRANSFORM_UNIFORMS], 1, GLU_FALSE, &mvp[0][0]);
}


GLuint Shader::CreateTextureShader(const std::string& text, unsigned int type)
{
...
std::string Shader::LoadTextureShader(const std::string& fileName)
{
	std::ifstream file;
