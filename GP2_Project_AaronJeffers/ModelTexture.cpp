...
void ModelTexture::initialise(const std::string& fileName)
{
	int texWidth, texHeight, noComponents; //width, height, and no of components of image
	unsigned char* imageData = stbi_load((fileName).c_str(), &texWidth, &texHeight, &noComponents, 4); //load the image and store the data
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texWidth, texHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData); //Target, Mipmapping Level, Pixel Format, Width, Height, Border Size, Input Format, Data Type of Texture, Image Data
...
void ModelTexture::TextureBind(unsigned int unit)
{
